...
- ```config/sessions.conf``` this file contains the combination of the ASN, the slug (e.g. the IXP name, or the name you want to contain in the filter name) and the router hostname/ip it's on.  You'll need to update this everytime you setup a new peer on an IX/PNI/New Router. You can also specify a 4th parameter of ```ipv4|ipv6``` if you wish to only push a configuration for a particular affinity, and an optional 5th parameter for a human-readable peer name. When a peer name is provided, it replaces ```as<ASN>``` in chain and filter file names (e.g. ```Kerfuffle-fcix-import-ipv4``` instead of ```as35008-fcix-import-ipv4```). Format: ```ASN,slug,router,affinity,peer_name``` — leave the 4th field empty if you want both address families with a peer name (e.g. ```35008,fcix,pr1.fmt2,,Kerfuffle```).
...
If you specify a peer name in ```sessions.conf```, the ASN portion is replaced with that name for more readable filters:

- ```Kerfuffle-fcix-import-ipv4``` for Kerfuffle at FCIX on IPv4
- ```Kerfuffle-fcix-import-ipv6``` for Kerfuffle at FCIX on IPv6
...
- You can help by making this better, submit feature requests, additions or questions
